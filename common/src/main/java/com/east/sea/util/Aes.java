...
public class Aes {
	...
	public static String encrypt(String key, String initVector, String data) {
		try {
			...
			throw new BusinessFrameException(BaseCode.AES_ERROR);
		}
	...
	public static String getAesByKey(String key) {
		...
		try {
			...
			ApiResponseString = byteToHexString(b);
			if(StringUtil.isNotEmpty(ApiResponseString) && ApiResponseString.length() > sixteen) {
				ApiResponseString = ApiResponseString.substring(sixteen);
			}
